...
void PosePublisher::registerPlugin()
{
    ...
    const char* attributes[] = {"frame_id", "pose_topic_name",
                                "vel_topic_name", "publish_rate",
                                "output_tf", "tf_child_frame_id"};
    ...
    plugin.reset = +[](const mjModel* m, double*, // plugin_state
        ...
        auto* plugin_instance =
            reinterpret_cast<class PosePublisher*>(plugin_data);
    ...
    plugin.compute =
        +[](const mjModel* m, mjData* d, int plugin_id, int // capability_bit
            ...
            auto* plugin_instance =
                reinterpret_cast<class PosePublisher*>(d->plugin_data[plugin_id]);
                                     ...
                                     int plugin_id)
{
    ...
    // pose_topic_name
    const char* pose_topic_name_char =
        mj_getPluginConfig(m, plugin_id, "pose_topic_name");
    ...
    // vel_topic_name
    const char* vel_topic_name_char =
        mj_getPluginConfig(m, plugin_id, "vel_topic_name");
    ...
    {
        if (!(strcmp(output_tf_char, "true") == 0 ||
              strcmp(output_tf_char, "false") == 0))
        {
    ...
    // tf_child_frame_id
    const char* tf_child_frame_id_char =
        mj_getPluginConfig(m, plugin_id, "tf_child_frame_id");
    ...
    return new PosePublisher(m, d, sensor_id, frame_id, pose_topic_name,
                             vel_topic_name, publish_rate, output_tf,
                             tf_child_frame_id);
}
...
{
    std::cout << "[PosePublisher] Configuring." << std::endl;
    ...
    {
        pose_pub_ = ros_context_->getNode()
                        ->create_publisher<geometry_msgs::msg::PoseStamped>(
                            pose_topic_name, 1);
        vel_pub_ = ros_context_->getNode()
