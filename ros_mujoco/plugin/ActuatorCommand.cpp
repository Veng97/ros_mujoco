...
void ActuatorCommand::RegisterPlugin() {
  ...
  // Allow plugins to be placed on either the body element or the actuator
  // element
...
ActuatorCommand *ActuatorCommand::Create(const mjModel *m, mjData *d,
  ...
  for (; actuator_id < m->nu; actuator_id++) {
    if (strcmp(actuator_name_char,
               mj_id2name(m, mjOBJ_ACTUATOR, actuator_id)) == 0) {
      break;
    }
  }
  if (actuator_id == m->nu) {
...
ActuatorCommand::ActuatorCommand(const mjModel *m,
  ...
  if (topic_name.empty()) {
    std::string actuator_name =
        std::string(mj_id2name(m, mjOBJ_ACTUATOR, actuator_id));
